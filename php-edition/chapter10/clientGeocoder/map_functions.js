...
function handleSearch() {
    ...
    // give us a hook for creating a "working" message
    changeBodyClass('geocoder-idle', 'geocoder-busy');
    
    geocoder.getLatLng(searchText,
        function (response) {
            changeBodyClass('geocoder-busy', 'geocoder-idle');
            ...
            if (!response) {
        		...
        		initializePoint(latlngs.length - 1);
        		redrawPolyline();
        		document.getElementById('s').value = ''; // clear the search box
            }
        }
    );
...
function init() {
	...
	document.getElementById('address-search').onsubmit = handleSearch;
	handleResize();
